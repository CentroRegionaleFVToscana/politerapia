...
if (TEST){
  testname <- "test_D5_Tabella_4_2025"
...
for (j in esenzioni) {
  ...
  covariates_binary <- c("iperpoliterapia")
...
for (j in esenzioni) {
  ...
  saveRDS(get(paste0("D5_", j)), file = paste0(thisdiroutput, "/D5_Tabella_4_caratterizzazione_",year, "_", j, ".rds"))
  write.csv(get(paste0("D5_", j)), file = paste0(thisdirexp, "/D5_Tabella_4_caratterizzazione_",year,"_", j, ".csv"))
